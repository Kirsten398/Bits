...
public class runButtonTwo : MonoBehaviour {
    ...
    private List<GameObject> startLasers;
    private List<GameObject> endLasers;

    // Use this for initialization
	void Start () {
        startLasers = new List<GameObject>();
        ...
        for (int i = 0; i < 2; i++)
        {
            startLasers.Add(Instantiate(laser, new Vector3(0, -2 + i, 1), laser.transform.rotation));
        }

        for(int j = 0; j < 2; j++)
        {
            endLasers.Add(Instantiate(laser, new Vector3(0, 1 + j, 1), laser.transform.rotation));
        }
    ...
    public void RunProgram()
    {
        if (blueSignal)
        {
            foreach (GameObject l in startLasers)
            {
        ...
        if (redSignal)
        {
            foreach (GameObject n in startLasers)
            {
                n.GetComponent<Renderer>().material = redMat;
            }

            foreach (GameObject o in endLasers)
            {
                o.GetComponent<Renderer>().material = blueMat;
            }
